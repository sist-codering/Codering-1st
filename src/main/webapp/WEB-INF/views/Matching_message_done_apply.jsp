...
<script type="text/javascript">
	...
	$(document).ready(function() 
	{
		...
		{
			var APPLY_ID = $('input[name=applyId]').val();
			...
			else
			{
				$.ajax({
		           ...
		           type : "POST",
		           url : "<c:url value = '/tutee_msg_apply_send_ajax' />",
		...
		{
			var APPLY_ID = $('input[name=applyId]').val();
			location.href = '<%=cp%>/tutee_msg_confirm_apply?APPLY_ID=' + APPLY_ID;
	...
	function sentMsg()
	{
		...
		$.ajax({
			type : "POST",
			url : "<c:url value='/tutee_msg_apply_ajax' />",
			...
			success : function(data)
			{
				...
				do
				{
					if (data[j].WRITER != NICKNAME)
					{
						strInfo += "<h5 class='title title_name'>튜터 "+ data[j].WRITER +" 님</h5>"
						strInfo += "<input type='hidden' name='writer' value=' "+ data[j].WRITER +" ' />";
						break;
					}
